...
struct Datos //estructura para recabar los datos del usuario
{
    ...
    char productos[8000]; //variable de productos para la memoria compartida
    ...
    else
    {
        ...
        while(fgets(linea, sizeof(linea), archivo))
        {
            ...
            if(strstr(linea, "Usuario: ") != NULL)
            {
                ...
                // ─── CAMBIO AQUÍ: Comparamos el correo del archivo contra el correo enviado ───
                if(strcmp(correo_archivo, memoria->correo) == 0)
                {
                    // Marcamos encontrado (Al no escribirlo en temp.txt, queda eliminado)
                    encontrado = 1;
                }
                else
                {
                    // Reescribimos el usuario que NO se elimina
                    fprintf(temp,"Usuario: %s\n", usuario_archivo);
                    fprintf(temp,"Password: %s\n", password_archivo);
                    fprintf(temp,"Correo: %s\n", correo_archivo);
                    fprintf(temp,"------------------------------------\n");
        ...
        // Verificamos si el usuario fue encontrado por su correo
        if(encontrado)
        {
